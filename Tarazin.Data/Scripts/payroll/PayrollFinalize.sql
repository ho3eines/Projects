-- =============================================
-- Tarazin.Data/Scripts/payroll/PayrollFinalize.sql
-- Schema: payroll
...
BEGIN TRAN;
    ...
    SELECT
        @Period,
        (SELECT COUNT(DISTINCT s.EmployeeId) FROM [payroll].[SalaryItems] s
         INNER JOIN [payroll].[Employees] e ON e.EmployeeId = s.EmployeeId AND e.IsDeleted = 0
         WHERE s.Period = @Period AND (@CompanyId IS NULL OR e.CompanyId = @CompanyId)),
        (SELECT ISNULL(SUM(CASE WHEN s.IsDeduction = 1 THEN -s.Amount ELSE s.Amount END), 0)
         FROM [payroll].[SalaryItems] s
         INNER JOIN [payroll].[Employees] e ON e.EmployeeId = s.EmployeeId AND e.IsDeleted = 0
         WHERE s.Period = @Period AND (@CompanyId IS NULL OR e.CompanyId = @CompanyId)),
        N'Finalized', SYSUTCDATETIME(), @CreatedBy, @CompanyId;
    ...
    VALUES (N'PayrollFinalized', CONCAT(N'RunId=', @Rid),
        (SELECT @Rid AS RunId, @Period AS Period, @EmpCount AS EmployeeCount, @Net AS NetTotal, @CompanyId AS CompanyId
