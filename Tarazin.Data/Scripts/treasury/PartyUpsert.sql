-- =============================================
-- Tarazin.Data/Scripts/treasury/PartyUpsert.sql
-- Schema: treasury | Cross-schema: central, accounting
-- Execute. ثبت/ویرایش طرف حساب یکپارچه (مشتری/تأمین‌کننده) + لینک خودکار حسابداری.
-- همان الگوی طلافروشی: گروه تفصیلی از تنظیمات سراسری شرکت خوانده می‌شود و
-- تفصیلی با کد طرف‌حساب ساخته/بازیابی شده و به معین پیش‌فرض گروه لینک می‌شود.
-- لینک در [treasury].[PartyLinks] ذخیره می‌شود تا همهٔ ماژول‌ها (خزانه/طلا/ارز)
-- از یک تعریف مشترک استفاده کنند.
-- =============================================
...
BEGIN
    -- گروه تفصیلی: اول تنظیمات سراسری شرکت، سپس تنظیمات خزانه
    DECLARE @GrpId INT = CASE WHEN @PartyType = N'Customer'
        THEN ISNULL((SELECT CustomerAccountGroupId FROM [accounting].[CompanyAccountSettings] WHERE CompanyId=@CompanyId),
                    (SELECT CustomerAccountGroupId FROM [treasury].[TreasurySettings] WHERE CompanyId=@CompanyId))
        ELSE ISNULL((SELECT SupplierAccountGroupId FROM [accounting].[CompanyAccountSettings] WHERE CompanyId=@CompanyId),
                    (SELECT SupplierAccountGroupId FROM [treasury].[TreasurySettings] WHERE CompanyId=@CompanyId)) END;
