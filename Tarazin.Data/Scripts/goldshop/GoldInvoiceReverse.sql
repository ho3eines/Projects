...
-- ۱) برگرداندن انبار
DECLARE @InvItemId INT,@InvWHId INT,@InvQty DECIMAL(18,4),@InvPrice DECIMAL(18,2),@MovementType NVARCHAR(20),@InvCost DECIMAL(18,2);
DECLARE curInv CURSOR LOCAL FAST_FORWARD FOR
    SELECT m.ItemId,m.WarehouseId,m.Qty,m.UnitPrice,m.MovementType,m.CostPrice,m.MovementId
    FROM [inventory].[Movements] m
    WHERE m.CompanyId=@CompanyId AND m.Description LIKE N'%'+@InvNum+N'%'
      AND m.IsDeleted=0;
OPEN curInv;
DECLARE @InvMvId INT;
FETCH NEXT FROM curInv INTO @InvItemId,@InvWHId,@InvQty,@InvPrice,@MovementType,@InvCost,@InvMvId;
...
BEGIN
    ...
    BEGIN
        ...
        -- لایهٔ بازگشتی (با قیمت تمام‌شدهٔ حواله)
        ...
        VALUES (@InvItemId,@InvWHId,NULL,@InvMvId,@InvQty,ISNULL(@InvCost,@InvPrice),CAST(SYSDATETIME() AS DATE),@CompanyId);
    END
    ELSE IF @MovementType=N'Receipt'
    BEGIN
        UPDATE [inventory].[Items] SET StockQty=StockQty-@InvQty,UpdatedAt=SYSUTCDATETIME()
        WHERE ItemId=@InvItemId AND CompanyId=@CompanyId;
        -- لایهٔ متناظر رسید حذف/صفر می‌شود
        UPDATE [inventory].[StockLayers] SET QtyRemaining=0
        WHERE ReceiptMovementId=@InvMvId AND CompanyId=@CompanyId;
    END
    FETCH NEXT FROM curInv INTO @InvItemId,@InvWHId,@InvQty,@InvPrice,@MovementType,@InvCost,@InvMvId;
END
...
-- ۴) برگرداندن چک — چک باطل می‌شود (Void).
-- بر اساس SourceReference (برای فاکتورهای جدید) یا تطبیق مبلغ/تاریخ (قدیمی).
UPDATE [treasury].[Cheques]
SET Status=N'Voided',UpdatedAt=SYSUTCDATETIME()
WHERE CompanyId=@CompanyId AND Status=N'Pending'
  AND (SourceReference=CONCAT(N'GoldInvoice:',@InvoiceId)
       OR (SourceReference IS NULL
