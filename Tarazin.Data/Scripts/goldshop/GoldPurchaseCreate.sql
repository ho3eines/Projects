-- =============================================
-- Tarazin.Data/Scripts/goldshop/GoldPurchaseCreate.sql
-- Schema: goldshop
-- Execute. فاکتور خرید — چندردیفه (طلا + ارز) با تسویهٔ ترکیبی.
--   آینهٔ GoldInvoiceCreate با جهت مخالف:
--     - طرف حساب باید Vendor باشد؛ تأمین‌کننده بستانکار می‌شود.
--     - ردیف‌های طلا → رسید انبار؛ ردیف‌های ارز → ورود به کیف پول شرکت.
--     - تسویه: نقدی/بانک/چک/ارز/طلا/نسیه (= باقیمانده، تراز صفر).
--     - سند حسابداری با Status='Note' (سند یاداشت)؛ بدهکار موجودی،
--       بستانکار: نقد/بانک/موجودی(طلا پرداختی)/تأمین‌کننده(نسیه).
-- =============================================
...
DECLARE @VendorName NVARCHAR(200)=(SELECT FullName FROM [central].[Parties] WHERE PartyId=@PartyId AND CompanyId=@CompanyId AND PartyType=N'Vendor' AND IsDeleted=0);
IF @VendorName IS NULL THROW 51080, N'تأمین‌کننده معتبر یافت نشد.', 1;
...
VALUES (N'',@InvoiceDate,@VendorName,@PartyId,ISNULL((SELECT TOP 1 ItemCode FROM @Lines WHERE RowType=N'Gold'),N'CURRENCY'),
        @GoldQty,@GoldWorkmanship,@GoldProfit,@TotalTax,@Total,N'Purchased',N'IRR',
...
DECLARE @InvoiceNumber NVARCHAR(50)=N'GPUR-'+RIGHT(N'00000'+CAST(@InvoiceId AS NVARCHAR(10)),5);
...
-- ── دفتر طرف‌حساب (بستانکار ریالی/طلا/ارز) ───
...
VALUES (@CompanyId,@PartyId,@InvoiceId,@InvoiceDate,N'Purchase',@Payments,@Total,ISNULL(@PayGoldGram,0),@GoldQty,
        ISNULL(@PayCurrencyQty,0),@FxRowQty,COALESCE(@PayCurrencyCode,@FxRowCode),@CreatedBy,
        N'خرید ' + @InvoiceNumber + CASE WHEN @FxRowQty>0 THEN N' + ارز '+@FxRowCode ELSE N'' END
...
-- ── رسید انبار (هر ردیف طلا) + ساخت کالا در صورت نیاز ──
IF @WarehouseId IS NULL THROW 51091, N'انبار پیش‌فرض طلافروشی در تنظیمات شرکت تنظیم نشده است.', 1;
DECLARE @GL NVARCHAR(50),@GLQty DECIMAL(18,4),@GLPrice DECIMAL(18,2),@GLCode NVARCHAR(50),@GLTitle NVARCHAR(200);
DECLARE curGold CURSOR LOCAL FAST_FORWARD FOR SELECT ItemCode,Qty,Price,Title FROM @Lines WHERE RowType=N'Gold';
OPEN curGold;
FETCH NEXT FROM curGold INTO @GL,@GLQty,@GLPrice,@GLTitle;
...
BEGIN
    ...
    BEGIN
        INSERT INTO [inventory].[Items](ItemCode,ItemTitle,Unit,StockQty,IsActive,IsDeleted,CreatedAt,UpdatedAt,CompanyId)
        VALUES (@GLCode,@GLTitle,N'گرم',0,1,0,SYSUTCDATETIME(),SYSUTCDATETIME(),@CompanyId);
    END
    ...
    VALUES (N'',N'Receipt',@InvItemId,@WarehouseId,NULL,@GLQty,@GLPrice,@GLPrice,@InvoiceDate,N'رسید بابت '+@InvoiceNumber,N'Posted',@CreatedBy,@CompanyId);
    ...
    VALUES (@InvItemId,@WarehouseId,NULL,@MvId,@GLQty,@GLPrice,@InvoiceDate,@CompanyId);
...
-- ── ورود ارز به کیف پول (ردیف‌های ارز) ───────
...
WHILE @@FETCH_STATUS=0
BEGIN
    IF @FXC IN (N'IRR',N'TOMAN') THROW 51082, N'برای ردیف ارز فقط ارز خارجی مجاز است.', 1;
    DECLARE @WQ DECIMAL(18,4)=ISNULL((SELECT Quantity FROM [currency].[Wallets] WHERE CurrencyCode=@FXC AND CompanyId=@CompanyId),0);
    DECLARE @WA DECIMAL(18,2)=ISNULL((SELECT AvgBuyRate FROM [currency].[Wallets] WHERE CurrencyCode=@FXC AND CompanyId=@CompanyId),0);
    DECLARE @NAvg DECIMAL(18,2)=CASE WHEN @WQ<=0 OR @WA<=0 THEN @FXR ELSE ROUND((@WQ*@WA+@FXQ*@FXR)/(@WQ+@FXQ),0) END;
    IF EXISTS (SELECT 1 FROM [currency].[Wallets] WHERE CurrencyCode=@FXC AND CompanyId=@CompanyId)
        UPDATE [currency].[Wallets] SET Quantity=Quantity+@FXQ,AvgBuyRate=@NAvg,InQty=InQty+@FXQ,LastMovementAt=SYSUTCDATETIME(),UpdatedAt=SYSUTCDATETIME(),UpdatedBy=@CreatedBy
    ...
    ELSE
        ...
        VALUES (@FXC,@FXQ,@NAvg,0,@FXQ,0,SYSUTCDATETIME(),@CreatedBy,@CompanyId);
    ...
    VALUES (N'',@InvoiceDate,CONVERT(TIME(0),SYSUTCDATETIME()),N'Buy',N'In',@FXC,@FXQ,@FXR,ROUND(@FXQ*@FXR,0),
            @VendorName,CONCAT(N'GOLDPUR:',@InvoiceId),N'خرید ارز بابت '+@InvoiceNumber,@CreatedBy,@CompanyId);
    FETCH NEXT FROM curFx INTO @FXC,@FXQ,@FXR;
END
CLOSE curFx; DEALLOCATE curFx;

-- ── خروج ارز پرداختی به تأمین‌کننده از کیف پول ─
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM [currency].[Wallets] WHERE CurrencyCode=@PayCurrencyCode AND CompanyId=@CompanyId)
    BEGIN DECLARE @NoWalletMsg NVARCHAR(2048)=N'کیف پول ' + ISNULL(@PayCurrencyCode,N'') + N' برای شرکت فعال تعریف نشده است.'; THROW 51078, @NoWalletMsg, 1; END
    IF (SELECT Quantity FROM [currency].[Wallets] WHERE CurrencyCode=@PayCurrencyCode AND CompanyId=@CompanyId) < @PayCurrencyQty
    BEGIN DECLARE @LowWalletMsg NVARCHAR(2048)=N'موجودی کیف پول ' + ISNULL(@PayCurrencyCode,N'') + N' برای پرداخت کافی نیست.'; THROW 51079, @LowWalletMsg, 1; END
    UPDATE [currency].[Wallets]
    SET Quantity=Quantity-@PayCurrencyQty,OutQty=OutQty+@PayCurrencyQty,LastMovementAt=SYSUTCDATETIME(),UpdatedAt=SYSUTCDATETIME(),UpdatedBy=@CreatedBy
    WHERE CurrencyCode=@PayCurrencyCode AND CompanyId=@CompanyId;
    INSERT INTO [currency].[CurrencyMovements]
        (MovementNumber,MovementDate,MovementTime,MovementType,Direction,CurrencyCode,Quantity,Rate,AmountRial,CounterPartyName,SourceReference,Description,CreatedBy,CompanyId)
    VALUES (N'',@InvoiceDate,CONVERT(TIME(0),SYSUTCDATETIME()),N'Pay',N'Out',@PayCurrencyCode,@PayCurrencyQty,@PayRate,@PayCurrencyValue,
            @VendorName,CONCAT(N'GOLDPUR:',@InvoiceId),N'پرداخت ارز بابت تسویه '+@InvoiceNumber,@CreatedBy,@CompanyId);
END

-- ── خروج طلای پرداختی به تأمین‌کننده (تسویه با طلا) ──
...
BEGIN
    ...
    IF @InvItemId2 IS NULL OR (SELECT StockQty FROM [inventory].[Items] WHERE ItemId=@InvItemId2 AND CompanyId=@CompanyId) < @PayGoldGram
        THROW 51075, N'موجودی انبار برای پرداخت طلا کافی نیست.', 1;
    -- مصرف لایه‌های موجودی (FIFO) — هماهنگ با ماژول انبار
    DECLARE @RemQty2 DECIMAL(18,4)=@PayGoldGram, @TotCost2 DECIMAL(18,2)=0, @LyId2 INT, @LyCost2 DECIMAL(18,2), @LyRem2 DECIMAL(18,4);
    DECLARE curLayers2 CURSOR LOCAL FAST_FORWARD FOR
        SELECT LayerId,UnitCost,QtyRemaining FROM [inventory].[StockLayers]
        WHERE ItemId=@InvItemId2 AND WarehouseId=@WarehouseId AND QtyRemaining>0 AND CompanyId=@CompanyId
        ORDER BY ReceivedDate,LayerId;
    OPEN curLayers2;
    FETCH NEXT FROM curLayers2 INTO @LyId2,@LyCost2,@LyRem2;
    WHILE @@FETCH_STATUS=0 AND @RemQty2>0
    BEGIN
        IF @LyRem2>=@RemQty2
        BEGIN SET @TotCost2=@TotCost2+@RemQty2*@LyCost2; UPDATE [inventory].[StockLayers] SET QtyRemaining=QtyRemaining-@RemQty2 WHERE LayerId=@LyId2; SET @RemQty2=0; END
        ELSE
        BEGIN SET @TotCost2=@TotCost2+@LyRem2*@LyCost2; SET @RemQty2=@RemQty2-@LyRem2; UPDATE [inventory].[StockLayers] SET QtyRemaining=0 WHERE LayerId=@LyId2; END
        FETCH NEXT FROM curLayers2 INTO @LyId2,@LyCost2,@LyRem2;
    END
    CLOSE curLayers2; DEALLOCATE curLayers2;
    DECLARE @PayIssueCost DECIMAL(18,2)=CASE WHEN @PayGoldGram>0 THEN ROUND(@TotCost2/@PayGoldGram,2) ELSE 0 END;
    INSERT INTO [inventory].[Movements]
        (MovementNumber,MovementType,ItemId,WarehouseId,SubWarehouseId,Qty,UnitPrice,CostPrice,MovementDate,Description,Status,CreatedBy,CompanyId)
    VALUES (N'',N'Issue',@InvItemId2,@WarehouseId,NULL,@PayGoldGram,@PayIssueCost,@PayIssueCost,@InvoiceDate,N'پرداخت طلا بابت تسویه '+@InvoiceNumber,N'Posted',@CreatedBy,@CompanyId);
...
BEGIN
    ...
    VALUES(@ChequeNumber,@ChequeBankId,@ChequeAmt,ISNULL(@ChequeDueDate,@InvoiceDate),N'Out',N'Pending',SYSUTCDATETIME(),@CreatedBy,@CompanyId);
END
IF ISNULL(@PayCash,0)>0
BEGIN
    IF @TreasuryCashBoxId IS NULL THROW 51083, N'صندوق پیش‌فرض طلافروشی تنظیم نشده است.', 1;
    ...
    VALUES (N'',@InvoiceDate,N'Out',@PayCash,N'IRR',NULL,@TreasuryCashBoxId,N'پرداخت نقدی '+@InvoiceNumber,CONCAT(N'GoldPurchase:',@InvoiceId),N'Posted',@CreatedBy,@CompanyId);
...
BEGIN
    IF @TreasuryBankId IS NULL THROW 51083, N'حساب بانکی پیش‌فرض طلافروشی تنظیم نشده است.', 1;
    INSERT INTO [treasury].[CashMovements]
        (MovementNumber,MovementDate,Direction,Amount,CurrencyCode,AccountId,CashBoxId,Description,SourceReference,Status,CreatedBy,CompanyId)
    VALUES (N'',@InvoiceDate,N'Out',@PayBank,N'IRR',@TreasuryBankId,NULL,N'پرداخت بانکی '+@InvoiceNumber,CONCAT(N'GoldPurchase:',@InvoiceId),N'Posted',@CreatedBy,@CompanyId);
...
-- ── سند حسابداری خرید (سند یاداشت — Status='Note') ─
...
SELECT @CashId=CashAccountId,@CashCode=CashAccountCode,@CashTitle=CashAccountTitle,
...
IF @PartyAccountId IS NULL OR @PartyCode IS NULL OR @InvId IS NULL OR @InvCode IS NULL
    THROW 51095, N'لینک حسابداری تأمین‌کننده یا حساب موجودی در تنظیمات شرکت تنظیم نشده است.', 1;
...
VALUES(RIGHT(N'00000000'+CAST(@NextNum AS NVARCHAR(10)),8),@InvoiceDate,N'Purchase',@VendorName,@Total,N'IRR',N'Note',@CreatedBy,0,@CompanyId,@FiscalYearId);
...
VALUES(@DocumentId,@InvId,@InvCode,@InvTitle,N'خرید طلا و ارز '+@InvoiceNumber,@Total,0);
...
IF @CashLine>0 AND @CashId IS NOT NULL AND @CashCode IS NOT NULL
    INSERT INTO [accounting].[DocumentLines](DocumentId,AccountId,AccountCode,Title,Description,Debit,Credit)
    VALUES(@DocumentId,@CashId,@CashCode,@CashTitle,CASE WHEN @PayCurrencyValue>0 THEN N'پرداخت نقدی و ارز '+@InvoiceNumber ELSE N'پرداخت نقدی '+@InvoiceNumber END,0,@CashLine);
IF ISNULL(@PayBank,0)>0 AND @BankId IS NOT NULL AND @BankCode IS NOT NULL
    INSERT INTO [accounting].[DocumentLines](DocumentId,AccountId,AccountCode,Title,Description,Debit,Credit)
    VALUES(@DocumentId,@BankId,@BankCode,@BankTitle,N'پرداخت بانکی '+@InvoiceNumber,0,@PayBank);
IF @GoldTradeValue>0 AND @InvId IS NOT NULL AND @InvCode IS NOT NULL
    INSERT INTO [accounting].[DocumentLines](DocumentId,AccountId,AccountCode,Title,Description,Debit,Credit)
    VALUES(@DocumentId,@InvId,@InvCode,@InvTitle,N'پرداخت طلا بابت تسویه '+@InvoiceNumber,0,@GoldTradeValue);
IF @ChequeAmt>0
    INSERT INTO [accounting].[DocumentLines](DocumentId,AccountId,AccountCode,Title,Description,Debit,Credit)
    VALUES(@DocumentId,@BankId,@BankCode,@BankTitle,N'پرداخت با چک '+ISNULL(@ChequeNumber,N'')+' '+@InvoiceNumber,0,@ChequeAmt);
IF @Remainder>0
    INSERT INTO [accounting].[DocumentLines](DocumentId,AccountId,AccountCode,Title,Description,Debit,Credit)
    VALUES(@DocumentId,@PartyAccountId,@PartyCode,@VendorName,N'نسیه '+@InvoiceNumber,0,@Remainder);
COMMIT;
