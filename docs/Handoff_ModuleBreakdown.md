...
- **تأیید لایو هم‌راستایی عمودی/افقی HTML↔PDF (۱۴۰۵/۰۶/۰۸)**: گزارش چک‌ها در هر دو حالت از دیزاینر بررسی شد: (۱) چاپ مرورگر — `window.print` بدون خطا؛ `@media print` فقط `.tpl-sheet` را نشان می‌دهد و `@page tpl-a4`/`@page tpl-a4-landscape` به‌درستی بر اساس جهت برگه فعال می‌شوند (`computedPage` در مرورگر تأیید شد)؛ (۲) QuestPDF — تولید PDF در هر دو حالت: عمودی 595×842pt و افقی 842×595pt (MediaBox با pymupdf تأیید شد)؛ (۳) **عرض ستون‌ها** با pymupdf از خطوط جدول استخراج و با درصدهای HTML مقایسه شد — در هر دو حالت و هر دو رندرر دقیقاً یکسان: [11.9, 11.9, 9.3, 11.9, 11.3, 11.9, 14.6, 17.2] (شماره چک→منبع). این تأیید نشان داد `RelativeColumn` در QuestPDF همان توزیع درصدی HTML را تولید می‌کند.
- **تأیید عرض نسبی ستون‌ها در PDF (۱۴۰۵/۰۶/۰۸)**: بررسی `PdfReportService.BuildTemplatePdf` نشان داد که برخلاف نسخهٔ قبلیِ HTML (عرض پیکسلی)، مسیر QuestPDF از همان ابتدا از **وزن نسبی** استفاده می‌کند — `RelativeColumn(Math.Max(col.Width, 20))` که دقیقاً همان توزیع درصدی HTML (`w/Σw`) است. تنها `ConstantColumn(24)` کل فایل برای ستون «ردیف» فاکتور طلافروشی است (مجاز). گارد تستی `BuildTemplatePdf_uses_relative_not_fixed_column_widths` در `PrintEngineTests` کد منبع را می‌خواند و (۱) وجود `RelativeColumn(Math.Max(col.Width, 20))` در `BuildTemplateTable`، (۲) نبود `ConstantColumn` در آن متد، و (۳) تنها مجاز بودن ConstantColumn در `BuildInvoiceLines` را assert می‌کند — تا این هم‌راستایی با HTML هرگز به عرض ثابت برنگردد.
...
- **رفع باگ QRCode مستقل از هدر شرکت (۱۴۰۵/۰۶/۰۹)**: `qr-fill` قبلاً فقط داخل `BuildCompanyHeader` رندر میشد — اگر کاربر «نمایش هدر اطلاعات شرکت» را خاموش میکرد، QR ناپدید میشد در حالی که سوییچ QR روشن میماند. اصلاح در `PrintSheetRenderer`: وقتی `ShowCompanyHeader=false` ولی `QrEnabled=true`، یک باند مستقل `.tpl-standalone-qr` (گوشهٔ بالای راست) ساخته میشود. دو باگ همراه هم رفع شد: (۱) `PrintDesigner` هیچ `OnAfterRenderAsync` برای `tarazinQr.fillAll` نداشت (برخلاف `DocumentPrintDialog`) — حالا بعد از هر رندرِ پیشنمایش QRها دوباره پر میشوند؛ (۲) SVG تولیدشدهٔ QR (۱۱۰px) از قاب ۴۴px بیرون میزد و باعث overflow میشد — `overflow:hidden` روی `qr-fill` + `max-width:100%` روی `qr-fill svg`. تأیید لایو در هر دو جهت (عمودی/افقی): هدر خاموش → `standalone=true`، `qrFilled=svg`، `overflows=false`. سه گارد تستی در `PrintEngineTests` اضافه شد: `BuildHtml_qr_renders_independent_of_company_header` (هدر خاموش → `tpl-standalone-qr` موجود و `qr-fill` با payload)، `BuildHtml_qr_hidden_when_disabled` (خاموش → هیچ `qr-fill`ای نباشد) و `BuildHtml_qr_inside_company_header_when_header_on` (هدر روشن → QR داخل هدر، نه standalone).
- **رفع overflow پیشنمایش عمودی (۱۴۰۵/۰۶/۰۸)**: در حالت عمودی (Portrait) محتوای پیشنمایش از صفحه بیرون میزد چون عرض کل ستونها (پیکسلی ثابت) از عرض صفحه بیشتر بود. اصلاح: (۱) `table-layout: fixed` به `.tpl-table` اضافه شد تا مرورگر عرض ستونها را متناسب با فضای موجود توزیع کند؛ (۲) `PrintSheetRenderer` عرض ستونها را به‌صورت **درصدی** (نسبت به کل) تولید میکند نه پیکسلی ثابت؛ (۳) `overflow: hidden` به `.tpl-sheet` و `.tpl-sheet-shell` اضافه شد تا محتوا از چارچوب بیرون نزند؛ (۴) `text-overflow: ellipsis` روی `th` و `td` برای جلوگیری از شکستن لیوت.
- **رفع «هدرِ بدون متن» در PDF قالب‌محور (۱۴۰۵/۰۶/۰۸)**: در `BuildTemplateTable` سلول‌های هدر فقط استایل می‌گرفتند و `Text(col.Title)` هرگز صدا زده نمی‌شد → همهٔ PDFهای موتور چاپ عمومی (چک‌ها، موجودی کالا، فروش روز، دوره‌های حقوق، سفارش‌ها و…) هدر ستون نداشتند (فقط نوار خاکستری خالی). با افزودن `.Text(col.Title)` بعد از `ColHeaderCell` هدرها در هر ۱۰ قالب ظاهر شدند؛ ترتیب RTL (اولین ستون راست‌ترین) و تراز هر ستون با HTML یکسان است. هم‌زمان عنوان گزارش که در هدر شرکت دوباره چاپ می‌شد (تکراری) حذف شد تا PDF دقیقاً مثل HTML یک‌بار عنوان را نشان دهد. تأیید: برون‌ریزی هر ۱۰ قالب با `pymupdf` — باند هدر بالای دیتا با ستون‌های قالب (مثل موجودی: کد→ارزش از راست به چپ)؛ تست `Template_renders_html_and_pdf` در `Tarazin.Tests/TemplateRenderGuards.cs` هر ۱۰ قالب را با هر دو رندرر HTML/QuestPDF در هر بیلد می‌سازد (نگهبان استثنای ساخت) و فایل‌ها را در `%TEMP%/tarazin-pdf/all` برون‌ریزی می‌کند.
...
- **گاردِ دروازهٔ چندفاکتوره (۱۴۰۵/۰۶/۱۰)**: `Tarazin.Tests/MultiInvoiceGateTests.cs` — قراردادِ یکپارچهٔ «هر گزارش/سفارش دقیقاً یک فاکتور + یک سند حسابداری + یک منبع یکتا» را با اسکریپت‌های REAL می‌قفل می‌کند: (۱) `accounting.SalesInvoiceFromOrder` — اجرای دوباره برای همان `OrderId` یک فاکتور جدید نمی‌سازد (idempotent on OrderId) و رویداد `InvoiceCreated` در Outbox هم تکراری نمی‌شود؛ (۲) `treasury.CashEntryFromInvoice` — اجرای دوباره برای همان فاکتور فقط یک `CashMovements` با `SourceReference='Invoice:{id}'` می‌سازد (ایندکس unique برقرار است)؛ (۳) سند حسابداریِ منسوب به هر سفارش فقط یک SourceReference یکتا دارد. گام «۲ج) گارد دروازهٔ چندفاکتوره» به `tools/run-checks.sh` و فیلترِ DB-guards در `.github/workflows/ci.yml` (کنار `PrintTemplate|AccountingCloseYear`) وصل شد.
...
- **UI/UX skeleton & component catalog**: `.claude/skills/tarazin-ui-ux/SKILL.md` lists every shared component (`PageHeader/PageToolbar/TzDataTable/StatCard/EntityActions/...`), the copy‑paste page skeletons (list/CRUD table, form, module dashboard, report) and the TarazinAccents palette — read it before creating any `.razor`.
- **Reporting / print**: `.claude/skills/tarazin-reporting/SKILL.md` defines the single reporting routine (report skeleton + `ReportPrintDialog` + `PdfReportService.BuildTablePdf` + `PdfFileNames` + `IPdfSaver`) on the single QuestPDF engine.
...
> **⚠️ Golden build rule:** after **any change in `Tarazin.Ui`** (any `.razor` page/component, service, or the shared `PdfReportService`), always run `dotnet build Tarazin.Web/Tarazin.Web.csproj`. `dotnet test` or a solo `Tarazin.Ui` build only refreshes `Tarazin.Ui/bin` — it does **not** refresh the `Tarazin.Web/bin` copy, and a dev server started with `--no-build` will keep serving the stale code (the recurring “columns RTL but header not” bug). Confirmed by `tools/check-stale-build.sh` before any dev-server restart. The one-shot gate `bash tools/run-checks.sh` runs this whole chain in sequence: cross-schema scan → **گاردهای قالب چاپ (فیلتر `PrintTemplate`)** → گارد برچسب منبع → کل سویت تست → **هفت گارد pymupdf (۴ب هدر راست‌چین، ۴ج A5L قالب، ۴د A5L جدول عمومی BuildTablePdf، ۴هـ بدون هدر/QR مستقل، ۴و چندصفحه‌گی BuildTablePdf، ۴ز A5L چندصفحه BuildInvoicePdf، ۴ط هدر فقط روی صفحاتِ دارای ردیف)** → بیلد وب → گارد stale. هر هفت گام pymupdf بدون نصب pymupdf/فایل‌های dump رسماً SKIP می‌شوند نه Fail (محلی اختیاری — در CI الزامی و چهار گارد — A5L قالب (۴ج)، چندصفحه‌گی جدول عمومی (۴و)، چندصفحه‌گی فاکتور طلا (۴ز) و هدر فقط روی صفحاتِ دارای ردیف (۴ط) — به‌صورت step جدا هم اجرا می‌شوند).

> **🤖 CI (GitHub Actions — `.github/workflows/ci.yml`)**: یک job واحد به نام `checks` که کل گیت را در خود دارد — (۱) گام اصلی همان `bash tools/run-checks.sh` را اجرا می‌کند؛ (۲) با سرویس `mcr.microsoft.com/mssql/server:2022-latest` و env `TARAZIN_TEST_CONN`، تست‌های DBدار همین‌جا واقعاً اجرا می‌شوند (و اگر SQL در دسترس نباشد Skip می‌شوند نه Fail)؛ (۳) دو step شرطی (`if: steps.wait-sql.outputs.ready == 'true'`) — اسکیمای واقعی از روی `Tarazin.Data/Scripts/*/_Ensure.sql` (با `docker cp` + `sqlcmd -i`، idempotent، پرچم `-I`) ساخته می‌شود و گاردهای DBدار (`FullyQualifiedName~PrintTemplate|AccountingCloseYearTests`) مقابل همان اسکیمای تازه دوباره اجرا می‌شوند. وقتی SQL بالا نیاید این دو گام Skip می‌شوند و گیت همچنان سبز است (مثل رفتار محلی). ⚠ علاوه بر این، CI **Python 3.12 + pymupdf نصب می‌کند** (`actions/setup-python` + `pip install pymupdf`) قبل از گام اصلی‌ چون `tools/check-rtl-headers.sh` بدون pymupdf SKIP می‌شود نه Fail. این اسکریپت (حالت‌ها: `all|generic|a5l|table|noheader|table-many|invoice-a5l-many|table-summary-pages`) هفت گارد نام‌دار دارد که هر کدام در `run-checks.sh` گام جدا است — **۴ب — هدر راست‌چین عمومی** (لوگوی سبز راست‌ترین روی فاکتور/چک/قالب/جدول)، **۴ج — گارد A5L قالب چاپ** (`template-a5l.pdf`)، **۴د — گارد BuildTablePdf A5L** (`table-a5l.pdf`)، **۴هـ — بدون هدر** (`template-a5l-noheader.pdf`: QR مستقل جایگزین لوگو)، **۴و — گارد BuildTablePdf چندصفحه‌گی** (`table-a5l-many.pdf`: جدول ۶۵ ردیفی باید چندصفحه باشد و هدر جدول در هر صفحه تکرار شود) و **۴ز — گارد BuildInvoicePdf A5L چندصفحه** (`invoice-a5l-many.pdf`: فاکتور ۲۵ ردیفی در A5L — MediaBox ۵۹۵×۴۲۰، چندصفحه، هدر در هر صفحه و بدون بیرون‌زدگی) و **۴ط — گارد هدر فقط روی صفحاتِ دارای ردیف** (`table-a5-summary.pdf`: جدول ۲۰ ردیفی در A5 پرتوره که صفحهٔ آخر فقط جمع‌بندی دارد — هدر باید روی هر صفحهٔ دارای ردیف باشد ولی صفحهٔ جمع‌بندی بدون هدر رسماً معتبر است؛ اشتباه قدیمی «هدر در هر صفحه» این حالت را Fail می‌کرد). ورودیِ همه، فایل‌های dumpای است که تست `Dump_rtl_header_pdfs_for_pymupdf` در `Tarazin.Tests/PrintRtlGuards.cs` می‌سازد: `%TEMP%/tarazin-pdf/rtl-headers/{invoice,cheque,template,template-a5l,table,table-a5l,template-a5l-noheader,table-a5l-many,invoice-a5l-many,table-a5-summary}.pdf`. هر گارد A5L (۴ج/۴د/۴هـ) MediaBox افقی ≈ ۵۹۵×۴۲۰ و هدر RTL (یا جایگزینی QR) را می‌سنجد؛ گاردهای ۴و/۴ز علاوه بر آن چندصفحه‌گی و تکرار هدر در هر صفحه را چک می‌کنند و ۴ز بیرون‌زدگی از لبه را هم می‌سنجد. با نصب pymupdf در CI این گام‌ها واقعاً اجرا و Fail واقعی می‌دهند؛ علاوه بر گام‌های داخل `run-checks.sh`، چهار گارد در workflow به‌صورت **step جدا و نام‌دار** هم اجرا می‌شوند: (الف) A5L قالب — `Run A5L template guard (pymupdf — MediaBox + RTL header)` → `bash tools/check-rtl-headers.sh a5l` (۴ج)؛ (ب) چندصفحه‌گی جدول عمومی — `Run BuildTablePdf multi-page guard (pymupdf — table-many)` → `bash tools/check-rtl-headers.sh table-many` (۴و)؛ (ج) چندصفحه‌گی فاکتور طلا — `Run BuildInvoicePdf multi-page guard (pymupdf — invoice-a5l-many)` → `bash tools/check-rtl-headers.sh invoice-a5l-many` (۴ز)؛ (د) هدر فقط روی صفحاتِ دارای ردیف — `Run table-header-on-table-pages guard (pymupdf — table-summary-pages)` → `bash tools/check-rtl-headers.sh table-summary-pages` (۴ط). اگر pymupdf یا فایل‌های dump نباشند، این گام‌ها رسماً SKIP می‌شوند (exit 0) و گیت همچنان سبز می‌ماند. راه‌اندازی محلی معادل: `bash tools/run-checks.sh` + اسکریپت‌های `_Ensure` روی یک SQL Server زنده. ⚠ تجربهٔ واقعی: اولین run CI در گام init شکست خورد چون (۱) `sqlcmd` پیش‌فرض QUOTED_IDENTIFIER OFF دارد و ایندکس‌های فیلترشده ساخته نمی‌شوند — پرچم `-I` لازم است؛ (۲) `accounting/_Ensure` و `currency/_Ensure` در حالت دیتابیسِ کاملاً تازه، قبل از ADD ستون از ستون استفاده می‌کردند و ارجاع literal به `goldshop.GoldShopSettings` در بچ‌کامپایل خطا می‌داد — همگی در commit `c30d98d` رفع شدند (جابه‌جایی بلاک‌ها + dynamic SQL) و run بعدی (#33264144264) با هر دو job سبز شد.
...
- `docs/BI_MODULE.md` – BI module: command center, alerts, and reports/print on the shared QuestPDF print engine.
...
- `.claude/skills/tarazin-ui-ux/SKILL.md` – **UI/UX guide** (shared-component catalog §2 + copy‑paste page skeletons §3 + TarazinAccents palette + RTL/dark/responsive). Read before creating or editing any `.razor` page.
- `.claude/skills/tarazin-reporting/SKILL.md` – **reporting/print guide** (unified report skeleton, `ReportPrintDialog`, `PdfReportService.BuildTablePdf`, `PdfFileNames`, `IPdfSaver`, single QuestPDF engine). Read before building any report/print/PDF.
...
| BI | `/bi`، همهٔ تب‌ها، `/bi/reports` (چاپ با موتور عمومی QuestPDF) | سالم؛ کاتالوگ گزارش‌ها + چاپ/PDF با `TemplatePrintDialog` |
...
- بیلد Web و Data با ۰ هشدار و ۰ خطا پاس شد. بیلد MAUI Android با ۰ هشدار و ۰ خطا پاس شد. بیلد MAUI Windows با ۰ خطا پاس شد؛ هشدار `TZN0001` مربوط به layout فیزیکی Build بود و assetها در `filtered.layout.resfiles` ثبت شده‌اند؛ publish/pack و تست دستگاهی Windows/Android هنوز باید در device lab تکرار شود.
