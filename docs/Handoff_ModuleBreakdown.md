...
> **🤖 CI (GitHub Actions — `.github/workflows/ci.yml`)**: دو job — (۱) `checks` روی هر push/PR همان `bash tools/run-checks.sh` را اجرا می‌کند (بدون SQL Server؛ تست‌های DBدار Skip می‌شوند نه Fail)؛ (۲) `db-guards` با سرویس `mcr.microsoft.com/mssql/server:2022-latest` — اسکیمای واقعی از روی `Tarazin.Data/Scripts/*/_Ensure.sql` (با `docker cp` + `sqlcmd -i`، idempotent) ساخته می‌شود و گاردهای DBدار (`FullyQualifiedName~PrintTemplate|AccountingCloseYearTests`) واقعاً اجرا می‌شوند. راه‌اندازی محلی معادل: `bash tools/run-checks.sh` + اسکریپت‌های `_Ensure` روی یک SQL Server زنده.
