...
## معماری فعلی (ساده‌شده، ۱۴۰۵/۰۵/۲۹ — نسخهٔ bootstrap)
...
ورود در هر دو هاست **کاملاً یکسان** است: `AuthService` همان کوئری
`UserAuthenticate` و PBKDF2 محلی را اجرا می‌کند. تنها فرق MAUI: اگر رشتهٔ اتصال
هنوز در حافظه نیست (شروع سرد یا بعد از خروج)، یک‌بار نام کاربری/رمز با HTTPS به
`POST /api/mobile/connection` می‌رود؛ سرور همان اعتبار را بررسی می‌کند و رشتهٔ
اتصال خودش را رمزگذاری‌شده (AES-256 با کلید مشتق از همان رمز) برمی‌گرداند. MAUI
در حافظه رمزگشایی می‌کند و همان مسیر مستقیم `DbService` را اجرا می‌کند.
بنابراین: **اگر ورود وب سالم است و API در دسترس است، ورود MAUI نیز باید سالم
باشد.**
