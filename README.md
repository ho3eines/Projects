...
* مدیریت اتصال SQL در Web است: `ConnectionStrings:DefaultConnection` در `Tarazin.Web/appsettings.json` (مقدار placeholder برای توسعهٔ محلی) و در production با secret `TARAZIN_SQL_CONNECTION` override می‌شود. `bootstrap password` فقط از secret استقرار می‌آید.
