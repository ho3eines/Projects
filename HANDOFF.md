...
### ۱۴۰۵/۰۶/۱۲ — هم‌خانواده‌سازی Drawer NavMenu با subnav (زیرخطی جمع‌وجور)

- پیش‌زمینه: تب‌های `ModuleSubNav` (`.tz-subnav__tab`) قبلاً به «تب‌های زیرخطی جمع‌وجور»
  بازطراحی شدند (ارتفاع ۲۸px، بدون قرص/کادر، حالت فعال = رنگ آکورد + تینت ۸٪ + نشانگر
  کشویی ۲٫۵px؛ hover = تینت ۷٪). این نوبت همان زبان به منوی کناری (Drawer NavMenu) اعمال شد.
- `app.css` → `.tz-nav .mud-nav-link`: ارتفاع ۳۰px (قبلاً ~۳۶px با پدینگ ۸px)، فونت ۰٫۸۸rem،
  radius ۷px (مطابق تب)، بدون جابه‌جایی پدینگ hover؛ hover = تینت آکورد ۷٪ (قبلاً `padding-inline`).
- حالت فعال: به‌جای گرادیان طلایی + متن روشن، حالا رنگ آکورد + تینت ۸٪ + نوار باریک
  ۲٫۵px (inset، هم‌ضخامت نشانگر subnav) — یک سیستم رنگی واحد در سایدبار و زیرمنو.
- عنوان گروه‌ها (`mud-nav-group`) و آیکون‌ها کوچک‌تر شدند (۰٫۹rem / ۱٫۱rem) تا هم‌قد بقیه.
- موبایل: لینک‌ها ۴۲px → ۳۸px و گروه‌ها ۴۶px → ۴۲px (هدف لمسی حفظ شد).
- تأیید: براکت‌های CSS متعادل؛ رندر با هارنس استاتیک (app.css واقعی) چک شد — فعال/هاور
  دقیقاً همان الگوی subnav. تغییر صرفاً CSS است و با رفرش زنده می‌شود.
- چک موبایل (۳۹۰px، app.css واقعی): هر ۱۲ آیتم دقیقاً ۳۸px؛ طولانی‌ترین عنوان بدون سرریز؛
  سرریز افقی صفر — بدون fix لازم.

### ۱۴۰۵/۰۶/۱۲ — پولیش اپ‌بار و منوی کاربر (هم‌خانواده با ناوبری) — فقط app.css
- برند: لوگو ۳۲px (موبایل ۲۸) با گوشهٔ ۸px؛ عنوان ۰٫۹۸rem و زیرعنوان ۰٫۷۲rem/op .65.
- دکمه‌های آیکونی (منو/پوسته): ۳۴px دسکتاپ، ۴۰px در ≤800 (هدف لمسی).
- چیپ «نام ماژول» (`tz-appbar__mod`): قرص→radius ۷px + تینت ۹٪ از `currentColor`
  (accent ماژول همان‌جا) — مثل حالت فعالِ تب.
- انتخاب‌گرهای زمینه (`tz-fy-menu__chip` سال مالی/انبار): قرص‌های توپُر → دکمهٔ نازک:
  پس‌زمینه شفاف، خط ظریف (text-primary 22%)، radius ۷px، فونت ۰٫۷۸rem، min-height ۳۰
  (موبایل ۳۲)؛ hover تینت primary ۸٪ + خط/متن آکورد.
- دکمهٔ کاربر (`tz-user`): قرص ۹۹۹px حذف → radius ۷px، پدینگ جمع‌وجور؛ نام ۰٫۸۸rem؛
  آواتار گوشه‌دار ۸px (هم‌نشین لوگو)؛ hover تینت primary ۸٪.
- تأیید: app.css واقعی روی هارنس دسکتاپ/موبایل — چیپ‌ها شفاف + خط ظریف + radius ۷،
  آواتار ۸px، آیکون‌ها ۴۰px موبایل، برند ۲۸px موبایل، نام/زیرعنوان در ≤600 مخفی.
  hover را نمی‌توان synthetic شبیه‌سازی کرد (وابسته به pointer واقعی) — قاعده در CSS است.
- یادداشت: در پهنای مصنوعی ~500px (نه واقعی) چیپ ماژول ممکن است تنگ شود؛ در ≤600
  پنهان و در دسکتاپ واقعی ≥800 جا کافی دارد — باگ واقعی نیست.

### ۱۴۰۵/۰۶/۱۲ — رفع هم‌پوشانی نوشته‌های نوار بالا در موبایل (T-C6D3E1BF) — فقط app.css
- شکایت: «نوشته‌های نوار بالا روی هم هستند». اندازه‌گیری با هارنس (MudBlazor CSS واقعی +
  app.css واقعی، DOM دقیق MainLayout) در ۹ پهنای ۳۹۰ تا ۱۴۴۰px، هم‌پوشانی واقعی ۳۱px
  بین متن برند و چیپ «سال مالی» در ۳۹۰px پیدا شد + ~۵px سرریز لبهٔ چپ.
- ریشه: wrapper منوی چیپ (`.mud-menu` با `display:inline-flex`) در flexbox با
  `min-width:auto` کوچک‌تر از محتوا نمی‌شود؛ پس چیپ‌ها (سال+انبار) عرض طبیعی خود را
  نگه می‌داشتند و در RTL به داخل برند می‌رفتند.
- fix در `app.css`:
  1) `.tz-fy-menu { min-width: 0; }` → اجازهٔ کوچک‌شدن چیپ‌ها (مقدار با ellipsis).
  2) `@media (max-width: 480px)`: متن برند مخفی (فقط لوگو) + پدینگ چیپ ۰٫۵rem +
     حداکثر عرض مقدار `min(19vw, 5.5rem)`.
- تأیید پس از fix (همان هارنس): هم‌پوشانی صفر در همهٔ پهن‌ها؛ ۳۹۰px → بدون سرریز لبه،
  چیپ‌ها ۳۲px تک‌خط. Build 0 errors. تغییر صرفاً CSS است و با رفرش زنده می‌شود.

### ۱۴۰۵/۰۶/۱۲ — منوی همبرگری: drawer همیشه از راست باز می‌شود (T-ABD5F671)
- شکایت: «منو همبرگری باید سمت راست باز شود نه سمت چپ». بررسی: MudBlazor 9.8.0 در حالت
  RTL (MudRTLProvider RightToLeft=true) Anchor.Start را به pos-right ترجمه می‌کند؛ هر دو
  سرور زنده (65220/65221) در HTML پیش‌رندر mud-drawer-pos-right دارند و تحلیل پیکسل از
  اسکرین واقعی کاربر، نوار تیره drawer را در ۸۲–۹۲٪ عرض صفحه (راست) نشان داد — پس رفتار
  فعلی کد همان «راست» است؛ مشکل کاربر از یک circuit قدیمی (قبل از RTL-aware) بود که با
  رفرش تب حل می‌شود.
- تغییرات (MainLayout.razor):
  1) Anchor.Start → Anchor.Right صریح: تضمین فیزیکی راست بودن مستقل از آبشار RTL؛
     با CSS موبایل (آفست بسته‌شده با right و --mud-drawer-width-right) هم‌خوان است.
  2) اصلاح جهت swipe: منطق قدیمی برای drawer چپ بود (بستن با کشیدن به چپ)؛ حالا که
     drawer راست است، بستن = کشیدن به راست (delta>0) و بازکردن = کشیدن به چپ.
- Build 0 errors. نیاز به رفرش تب مرورگر (Ctrl+F5) برای اعمال روی circuit قدیمی.

### ۱۴۰۵/۰۶/۱۲ — جستجو در منوی کناری (drawer) (T-050B94D9)
- درخواست: «منو همبرگری قابلیت سرچ داشته باشد تا راحت به همه‌چیز برسیم».
- NavMenu.razor: کادر جستجوی چسبان (sticky) بالای منو اضافه شد (MudTextField با
  آیکون Search، Clearable و Escape برای پاک‌کردن). با تایپ متن، درخت عادی به لیست
  صافِ نتایج منطبق تبدیل می‌شود که بر اساس ماژول گروه‌بندی شده است؛ محدودیت‌های
  دسترسیِ درخت عادی (CanView/HasPermission/HideInNav) در نتایج هم اعمال می‌شود.
  اگر چیزی نیافت → پیام «چیزی مطابق … یافت نشد». با خالی‌شدن متن، درخت برمی‌گردد.
- نرمال‌سازی فارسی در تطبیق: ي/ك/أ/إ/آ/ة عربی و نیم‌فاصله تا جستجو خطا ندهد
  (مثلاً «ي» با «ی» یکی است).
- app.css: بلوک `.tz-nav__search` — sticky (top:0,z:4)، پس‌زمینه از
  --mud-palette-drawer-background، متن/placeholder از drawer-text، آیکون و خط
  focus از drawer-icon طلایی؛ هم‌خانواده با بقیهٔ ناوبری تیره.
- تأیید: build 0 errors؛ هارنس با CSS واقعی — متن ورودی drawer-text، کادر
  outline 30٪، placeholder 55٪، آیکون طلایی، sticky بعد از اسکرول کار می‌کند.

### ۱۴۰۵/۰۶/۱۲ — اسکرین‌شات واقعی مرورگر + تعویض سرور کهنه (T-0EDADB2B)
- کاربر دکمهٔ «اسکرین‌شات واقعی در مرورگر ویندوز» را انتخاب کرد. حین بررسی مشخص شد:
  پورت‌های 65220/65221 را یک باینری کهنه (Tarazin.Web.exe، PID 13100، شروع 15:26 با
  build 15:04) گرفته بود؛ همان سرور روی صفحهٔ خانهٔ کاربر NullReferenceException می‌داد
  (مشکل circuit قدیمی) و dotnet watch (Task Scheduler) به‌خاطر مودکس تک‌نمونه نمی‌توانست
  بایند شود.
- اقدام: باینری کهنه متوقف شد → watch دوباره build/بایند کرد → حالا کدِ جاری روی
  65220/65221 سرو می‌شود (مشاهده: pos-right، CSS جدید، home 200).
- تأیید بصری واقعی (Chrome تازه + UIA + OCR + اسکرین‌شات):
  1) صفحهٔ خانهٔ لاگین‌شده بدون خطا رندر می‌شود (سلام، کارت هشدار چک‌ها، کارت‌های ماژول).
  2) drawer سمت راست است؛ دکمهٔ همبرگری بالای راست؛ چیپ «۱۴۰۵» و نام کاربر چپ.
  3) کادر «جستجو در منو» در drawer واقعاً فیلتر می‌کند: درخت → لیست صاف ماژول‌های منطبق.
  4) صفحهٔ انبار (اسناد روز انبار) با فیلتر انبار/مبدأ و جدول ۲۹ ردیفی سالم است.
- شواهد: docs/screens/real-browser-20260903/ (dash-current.png, inventory-current.png,
  nav-search-live.png, evidence-nre-stale-server.png).
- تب قدیمی کاربر (در سرور قبلی) باید یک‌بار Ctrl+F5 شود تا circuit جدید بگیرد.

### ۱۴۰۵/۰۶/۱۲ — چک موبایل واقعی در مرورگر (T-54F5D5B2)
- درخواست: چک واقعی تلفن همراه (دکمهٔ «📱 چک موبایل واقعی»). Chrome واقعی با
  پنجرهٔ ۵۱۶×۸۴۴ (حداقل عرض پنجرهٔ ویندوز؛ media ≤800/≤600 فعال) به کد جاری
  (65220) وصل و با admin لاگین شد.
- نتایج (UIA + OCR + اسکرین‌شات):
  1) اپ‌بار موبایل: همبرگری بالا-راست؛ چیپ «۱۴۰۵» (x341) و «انبار اصلی» (x233)
     مجزا بدون هم‌پوشانی — fix قبلی (min-width:0 + برند فقط-لوگو در ≤480) پایدار.
  2) بازکردن drawer از راست روی overlay تیره انجام می‌شود و کادر «جستجو در منو»
     داخل آن است.
  3) صفحهٔ انبار (اسناد روز انبار) در عرض موبایل کامل رندر می‌شود (فیلتر از-تا/
     انبار/مبدأ + جستجو) — بدون خطا.
- شواهد: docs/screens/mobile-real-20260903/ (mobile-home/drawer/inventory-clean.png).
- نکته: صفحهٔ ورود/منوها در این پروفایل تم تیره را نشان داد؛ رندر تیره سالم بود و
  تم روشن در چک دسکتاپ تأیید شده — موضوع تم جداگانه نیست.

### ۱۴۰۵/۰۶/۱۲ — وضعیت سه تسک: چیپ‌ها، تست‌ها، انتقال به main (T-37A0984F / T-CF2F136E / T-6C6D1968)
- **T-37A0984F (رنگ چیپ‌های سال مالی/انبار) — انجام و تأیید شد.** چیپ‌ها روی اپبار
  تیره (#0B3A38 روشن / #061615 تیره) بودند ولی با `--mud-palette-text-primary` رنگ می‌خوردند
  (قهوه‌ای تیره #1C1914 در حالت روشن → تقریباً نامرئی). رفع در `app.css` + کلاس‌های
  modifier در `MainLayout.razor`: متن کرم `--mud-palette-appbar-text`، پس‌زمینهٔ شیشه‌ای ۷٪،
  و آیکون آکوردی — سال مالی طلایی `#E8C877` (`--year`)، انبار سبزآبی `#7EB8B0` (`--warehouse`).
- **T-40CCCE39 (چک بصری چیپ‌ها روی سرور) — انجام شد.** تأیید قطعی با CSS زندهٔ واقعی
  (`_content/Tarazin.Ui/css/app.css`) در DOM وفادار MudChip: آیکون سال = rgb(232,200,119)
  و آیکون انبار = rgb(126,184,176) — محاسبهٔ computed style؛ کپچر واقعی صفحهٔ خانه نیز
  چیپ‌ها را با متن/حاشیهٔ کرم روی اپبار نشان داد. (آیتم قبلیِ `--wh` اشتباه بود؛ نام کلاس
  واقعی `--warehouse` است.)
- **T-CF2F136E (اجرای کامل تست‌های انبار) — ۲۲/۲۲ سبز** در ۷ کلاس ایزوله.
- **T-DE951543 (تست + سنجش سرعت در DB خالی از دیتای نمونه) — انجام شد.** پس از
  seed-cleanup، همان ۷ کلاس جداگانه: Phase5 845ms، MovementInsert 2s، Adjustment 74ms،
  LotSerial 275ms، ItemPicker 132ms، SeedCleanup 1s، GoldItemLink 73ms (مجموع ~4.3s) —
  timeout موازیِ گذشته از شلوغی DB اشتراکی بود نه کندی کوئری.
- **T-6C6D1968 (انتقال به پروژهٔ اصلی) — انجام شد.** فقط همین چک‌اوت روی دستگاه است
  (`/d/Hermes/projects`، branch `main`)؛ تغییرات انباشته به‌صورت checkpoint کامیت شد:
  **b21e1eb** (CSS یکپارچه nav/appbar/chips، جستجوی drawer، TzNumericField+تست،
  شواهد مرورگر docs/screens/). `.gitignore` برای پروفایل Chrome آزمایشی و پیش‌نمایش‌های
  موقت اضافه شد تا commit نشوند؛ درخت پاک است.
- ابزار جدید: **tools/telegram_photo.py** — ارسال عکس به ربات از همان config/پروکسی
  agent (sendPhoto، ریتای). کاربرد: اسکرین‌شات هر صفحه برای کاربرِ دور (T-A099BC5E ارسال شد).
- دیباگ تونل (T-D4F0CA9E): درخواستِ فیلدشده T-A27BB5E1 بود — اپراتور DNS نام‌های
  edge کلادفلر را NXDOMAIN می‌کند؛ حتی از پروکسی 3067، edge-DNS خود cloudflared «no such
  host» می‌دهد و data-plane اش از HTTP-proxy رد نمی‌شود → گزینهٔ قابل‌اتکا همان ربات
  (کنترل + اسکرین‌شات) است.

### ۱۴۰۵/۰۶/۱۲ — تانل ارگو (cloudflared) بالاخره کار کرد (T-783D56A2)
- ریشه‌ٔ مشکل: اپراتور DNS رکوردهای SRV/A منطقه‌های edge کلادفلر
  (`region1/region2.v2.argotunnel.com`) را NXDOMAIN می‌کند؛ cloudflared آن‌ها را خودش
  resolve می‌کند (از HTTP-proxy رد نمی‌شود) → کوئیک‌تانل هر ۱۲–۳۵ ثانیه قطع می‌شد.
- راه‌حلِ کارا (ترکیب دو تکه):
  ۱) **hosts pinning** — در `C:\Windows\System32\drivers\etc\hosts` این خطوط اضافه شد
     (IPهای واقعی از DoH از طریق پروکسی 3067 گرفته شد):
     `198.41.192.47 / .77  region1.v2.argotunnel.com` و `198.41.200.63 / .73 region2.v2.argotunnel.com`
  ۲) اجرا با پروکسی + http2:
     `HTTPS_PROXY=http://127.0.0.1:3067 HTTP_PROXY=… ALL_PROXY=… TUNNEL_TRANSPORT_PROTOCOL=http2 \
      cloudflared tunnel --url https://localhost:65220 --no-autoupdate --no-tls-verify --edge-ip-version 4`
- نتیجه: ثبت تونل روی `198.41.200.63` (sjc08, http2) پایدار؛ تست HTTP 200 از آدرس عمومی؛
  آخرین آدرس: https://oak-races-sierra-slow.trycloudflare.com (موقت — با ری‌استارت عوض می‌شود).
- ⚠️ cloudflared به‌صورت nohup در حال اجراست؛ بعد از ری‌استارت باید با دستور بالا دوباره
  بالا بیاید. اگر کاربر «همیشگی‌کردن» را انتخاب کرد → Task Scheduler با آن envها.
- ابزار کمکی: `tools/telegram_photo.py <img> [caption]` برای فرستادن عکس به ربات.
- **تانل همیشگی شد (T-783D56A2 ادامه):** `tools/cloudflared-tunnel.sh` (با همان envها،
  kill نمونهٔ قبلی، استخراج URL و گزارش به تلگرام) + `tools/start-cloudflared-tunnel.bat`
  در Task Scheduler به‌نام **Tarazin\CloudflaredTunnel** (ONLOGON) ثبت شد. تست کامل: تونل
  جدید روی 198.41.200.63 (http2) ثبت و HTTP 200 از آدرس عمومی گرفت؛ پس از هر ری‌استارت
  URL جدید به ربات اعلام می‌شود. آخرین URL در /tmp/cf-tunnel.log است.

### ۱۴۰۵/۰۶/۱۳ — CSS از تانل نمی‌آمد (T-9782AAA7)
- نشانه: صفحهٔ HTML از تانل 200 می‌گرفت ولی همهٔ `_content/...` (CSS/JS) 000/502/530
  می‌دادند → ظاهر بدون استایل.
- ریشه‌های واقعی (دو تا):
  ۱) **دو نمونهٔ cloudflared هم‌زمان** (۴ پروسه = ۲ جفت shim+real، یکی از Task Scheduler و
     یکی از اجرای دستی قبلی wrapper) → تداخل و قطع/وصل مکرر edge.
  ۲) **سرور dev روی 65220 کرش کرده بود** (پورت خالی، فقط dotnet watch زنده بود؛ سوپروایزر
     گزارش «healthy» کهنه می‌داد چون چک ۳ ثانیه‌ای curl را رد کرده بود؟ در واقع سرویس child
     مرده بود و watch ری‌استارتش نکرده بود). log تانل: `dial tcp [::1]:65220: connectex:
     refused` — یعنی origin واقعاً پایین بود، نه مشکل CSS.
- رفع: همهٔ cloudflared را kill و فقط یک نمونه از `tools/cloudflared-tunnel.sh` بالا آمد
  (URL جدید: https://beats-byte-ride-ambien.trycloudflare.com) + `bash tools/start-dev-server.sh
  --force` برای ری‌استارت تمیز سرور (سوپروایزر قدیمی‌ها را پاک کرد).
- نتیجه: **۲۴/۲۴ درخواست** (HTML + app.css + MudBlazor.min.css + blazor.server.js ×۶) از تانل
  200؛ اسکرین‌شات مرورگر واقعی از تانل تأیید شد (چیپ‌ها/تم درست).
- درس: اگر CSS/assetها از تانل نمی‌آیند، اول `curl -sk https://localhost:65220/api/health`
  و `netstat | grep 65220` بزن — origin مرده شایع‌ترین علت است، نه cloudflared.
